...
if($identioficador != '')
{
	...
	$queryVISTAPREV = $match->Listado_MATCH($identioficador);
	$conn = $match->db();
	...
	echo ', <STRONG>TARJETA: </STRONG> AMERICAN EXPRESS. ';
?>
...
<td width="12.5%">'.$row['COLABORADOR_MATCH'].'</td>
<td width="12.5%">'.$row['TARJETA_MATCH'].'</td>
<td width="12.5%">'.$row['FECHADD_MATCH'].'</td>
<td width="12.5%">'.$row['ESTABLECIMIENTO_MATCH'].'</td>
<td width="12.5%">'. number_format( $row['MONTO_MATCH'],2,'.',',').'</td>
<td width="12.5%">'.$row['OBSERVACIONES_MATCH'].'</td>
<td width="12.5%">'.$row['FECHA_MATCH'].'</td>
<td width="12.5%">
...
<input type="checkbox" style="width:30px; color:red;" name="documentos'.$row["id"].'"  value="'.$row["id"].'" class="form-check-input" 
			...
			onclick="pasarmatchdocumento('.$identioficador.','.$row["id"].',\'AMERICANE\')" 
			'./*regresa checked*/$match->validaexistematchCOMPROBACION($identioficador,$row["id"],'AMERICANE').' 
			'./*regresa disabled*/$match->validaexistematch3resCOMPROBACION($row["id"],$identioficador,$permiso,'AMERICANE',$var_respuesta).'>
&nbsp;&nbsp;&nbsp;&nbsp;'.$match->validaexistematch4COMPROBACION($row["id"],'AMERICANE').'
</td>
</tr>
';
}
