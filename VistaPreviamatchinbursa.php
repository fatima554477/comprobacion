...
if($identioficador != '')
{
	...
	$queryVISTAPREV = $match->Listado_INBURSA($identioficador);
	$conn = $match->db();
	...
	echo ', <STRONG>TARJETA: </STRONG> INBURSA. ';
?>
...
<td width="12.5%">'.$row['COLABORADOR_INBURSA'].'</td>
<td width="12.5%">'.$row['TARJETA_INBURSA'].'</td>
<td width="12.5%">'.$row['FECHADD_INBURSA'].'</td>
<td width="12.5%">'.$row['ESTABLECIMIENTO_INBURSA'].'</td>
<td width="12.5%">'.$row['RFC_INBURSA'].'</td>
<td width="12.5%">'. number_format( $row['MONTO_INBURSA'],2,'.',',').'</td>
<td width="12.5%">'.$row['OBSERVACIONES_INBURSA'].'</td>
<td width="12.5%">'.$row['FECHA_INBURSA'].'</td>
<td width="12.5%">
...
<input type="checkbox" style="width:30px; color:red;" name="documentos'.$row["id"].'"  value="'.$row["id"].'" class="form-check-input" 
			...
			onclick="pasarmatchdocumento('.$identioficador.','.$row["id"].',\'INBURSA\')" 
			'./*regresa checked*/$match->validaexistematchCOMPROBACION($identioficador,$row["id"],'INBURSA').' 
			'./*regresa disabled*/$match->validaexistematch3resCOMPROBACION($row["id"],$identioficador,$permiso,'INBURSA',$var_respuesta).'>
&nbsp;&nbsp;&nbsp;&nbsp;'.$match->validaexistematch4COMPROBACION($row["id"],'INBURSA').'
</td>
</tr>
';
}
